...
Reruns workflow runs cancelled by `cancel-in-progress` when stacked-PR tools (notably [Graphite's `gt submit --stack`](https://graphite.dev/)) deliver two webhooks per submit, so the PR Checks sidebar reflects the surviving sibling's status.

## The problem

Some stacked-PR tools (notably Graphite's `gt submit --stack`) cause GitHub to deliver two `pull_request.synchronize` webhooks per PR — one for the git push, one for the tool's REST follow-up. With `cancel-in-progress: true` on a PR-scoped concurrency group, the two resulting workflow runs race and one gets killed within ~2 seconds.
...
```yaml
# .github/workflows/heal-zombie-cancels.yml
name: Heal zombie cancels
on:
  workflow_run:
